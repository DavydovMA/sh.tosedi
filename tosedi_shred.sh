...
SOFT_FILE="tosedi_shred.sh"
SOFT_NAME="sh.tosedi_shred"
SOFT_VERSION="20.01"
...
# sh.tosedi_shred
# - скрипт, зачищает на диске GPT: заголовки, таблицы и т.д..
...
echo "------------- ---- ------"
	echo	"WARNING!	: ALL DATA BE LOST!!! (YES/NO)"
if  test "${2}" = "--yes"  ;
then	# --yes {
	read_yesno="YES"
else	# : --yes
...
then	# dump_size_correct {
	...
	dd if=/dev/urandom of=${dump_disk} seek=${lba_now} bs=${size_sect} count=${size_dd} status=progress
	err_ret="${?}"
if test "${err_ret}" = "0" ;
then	# dd_part {
...
echo "------------- ---- ------"
	...
	dd if=/dev/urandom of=${dump_disk} seek=${lba_gpt2} bs=${size_sect} count=${size_dd} status=progress
	err_ret="${?}"
if test "${err_ret}" = "0" ;
then	# dd_gpt_2 {
	echo	"GPT-2		: [OK]"
...
echo "------------- ---- ------"
	...
	dd if=/dev/urandom of=${dump_disk} seek=${lba_gpt1} bs=${size_sect} count=${size_dd} status=progress
	err_ret="${?}"
if test "${err_ret}" = "0" ;
then	# dd_gpt_1 {
	echo	"GPT-1		: [OK]"
